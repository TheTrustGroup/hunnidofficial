...
**Warehouse binding:** The server binds each POS email to a warehouse (Main Jeff / Hunnid Main) and returns `user.warehouseId` in the login response. The POS UI then **skips the "Select warehouse to start" screen** and opens that warehouse’s POS directly. Super admin logins are not bound, so they still see the warehouse selector.
...
|-------|---------------------------|--------|
| `jcashier@hunnidofficial.com` | `POS_PASSWORD_CASHIER_MAIN_STORE` | Your chosen password (e.g. `MySecurePOS123`) |
| `hcashier@hunnidofficial.com` | `POS_PASSWORD_MAIN_TOWN` | Your chosen password |
...
2. In that project’s **Environment Variables**:
   - For **Main Jeff cashier:** `POS_PASSWORD_CASHIER_MAIN_STORE` = the password you want.
   - For **Hunnid Main cashier:** `POS_PASSWORD_MAIN_TOWN` = the password you want.
...
4. Try logging in again with:
   - Email: `jcashier@hunnidofficial.com` or `hcashier@hunnidofficial.com`
