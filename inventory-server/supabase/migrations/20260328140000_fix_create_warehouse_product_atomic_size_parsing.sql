-- Fix create_warehouse_product_atomic: old code used replace(sizeCode,' ','') which turned "O S" into "OS"
-- and broke sized products. Also skip zero-qty rows, normalize "EU 36.5" -> EU36.5, and persist color.
...
begin
  ...
  if jsonb_array_length(p_quantity_by_size) > 0 then
    ...
    loop
      ...
      -- Catalog uses EU36.5 not "EU 36.5" (do not strip all spaces — that creates OS from "O S")
