-- Data integrity: record_sale must not allow oversell (two cashiers selling last unit).
-- 1. Deduct only with WHERE quantity >= v_qty; raise INSUFFICIENT_STOCK if no row updated.
-- 2. Do all deductions first, then insert sale + lines (so full rollback on any failure).
...
BEGIN
  ...
  -- Phase 1: validate and deduct stock for every line (atomic; raises on first insufficient).
  ...
  -- Phase 2: insert sale and lines (all deductions already committed in this transaction).
  INSERT INTO sales (
...
COMMENT ON FUNCTION record_sale(uuid, jsonb, numeric, numeric, numeric, numeric, text, text, uuid) IS
  'Atomic sale: deduct stock (with sufficient-stock check) then insert sale+lines. Raises INSUFFICIENT_STOCK if any line would go negative.';
