-- Persist warehouse_products.cost_price on each sale_line at checkout (COGS at time of sale).
...
COMMENT ON FUNCTION record_sale_impl(uuid, jsonb, numeric, numeric, numeric, numeric, text, text, uuid, text, uuid) IS
  'Atomic sale with stock deduction; stores cost_price on sale_lines; optional p_client_event_id for idempotency.';
