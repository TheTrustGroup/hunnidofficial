-- Fix idempotency lookup in record_sale_impl.
-- When p_client_event_id is present but no existing row is found, keep generated v_sale_id.
-- Previously, SELECT ... INTO v_sale_id could overwrite it to NULL, causing:
-- "null value in column \"id\" of relation \"sales\" violates not-null constraint".
...
GRANT EXECUTE ON FUNCTION public.record_sale_impl(uuid, jsonb, numeric, numeric, numeric, numeric, text, text, uuid, text, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.record_sale_impl(uuid, jsonb, numeric, numeric, numeric, numeric, text, text, uuid, text, uuid) TO service_role;
