-- record_sale: accept both camelCase and snake_case keys in p_lines so inventory deduction
-- works regardless of how the client serializes the JSON. Ensures POS stock always deducts.
-- p_lines can be jsonb or text (API may send JSON string); we normalize to jsonb.
-- Run after 20250228170000_sales_sold_by_email.sql.
...
BEGIN
  ...
  LOOP
    -- Accept both camelCase and snake_case so client serialization never skips deduction
...
COMMENT ON FUNCTION record_sale(uuid, jsonb, numeric, numeric, numeric, numeric, text, text, uuid, text) IS
  'Atomic sale with stock deduction. Accepts camelCase or snake_case keys in p_lines. Raises INSUFFICIENT_STOCK or INVALID_LINE.';
