-- Paginated product list in one indexed query (avoids loading all SKUs + images before LIMIT).
      ...
      SELECT coalesce(
        json_agg(
          json_build_object(
            ...
            'supplier', wp.supplier,
            'tags', coalesce(wp.tags, '[]'::jsonb),
            'images', coalesce(wp.images, '[]'::jsonb),
            'version', wp.version,
...
COMMENT ON FUNCTION public.list_warehouse_products_page IS
  'Returns { total, data } for one warehouse page; used by GET /api/products.';

GRANT EXECUTE ON FUNCTION public.list_warehouse_products_page(uuid, int, int, text, text, text, text) TO service_role;
GRANT EXECUTE ON FUNCTION public.list_warehouse_products_page(uuid, int, int, text, text, text, text) TO authenticated;
