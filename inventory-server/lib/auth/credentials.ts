/**
 * Validate email + password against env (ALLOWED_ADMIN_EMAILS, POS_PASSWORD_*, ALLOWED_POS_EMAILS).
 * Used by POST /api/auth/login and POST /admin/api/login.
 *
 * POS logins:
 * - When ALLOWED_POS_EMAILS is set, the login route may validate via Supabase Auth first (each cashier's own password in Supabase). If Supabase is not used or fails, env POS_PASSWORD is used as fallback.
...
/** Comma-separated POS cashier emails (e.g. Hunnid: hcashier@hunnidofficial.com,jcashier@hunnidofficial.com). When set, these log in with POS_PASSWORD. */
const ALLOWED_POS_EMAILS_ENV = 'ALLOWED_POS_EMAILS';
/** Single password for all emails in ALLOWED_POS_EMAILS. Required when ALLOWED_POS_EMAILS is set. */
const POS_PASSWORD_ENV = 'POS_PASSWORD';
