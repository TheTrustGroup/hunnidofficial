-- Harden record_sale: guard empty p_lines, require size_code for sized products, accept p_lines as text or jsonb.
-- Ensures we never insert a sale without deducting stock, and sized products always deduct by size.
-- Run after 20260304160000_record_sale_accept_camel_and_snake.sql.

-- Implementation shared by both overloads (jsonb and text).
...
DECLARE
  ...
  v_qty_raw     text;
BEGIN
  -- Guard: never process empty lines (prevents sale without deduction).
  ...
  LOOP
    ...
    -- Sized products must have size_code so we deduct from warehouse_inventory_by_size.
...
-- Drop existing record_sale(jsonb) and replace with wrapper that calls impl.
DROP FUNCTION IF EXISTS record_sale(uuid, jsonb, numeric, numeric, numeric, numeric, text, text, uuid, text);

-- Public record_sale: jsonb (client sends array).
...
AS $$
BEGIN
  RETURN record_sale_impl(p_warehouse_id, p_lines, p_subtotal, p_discount_pct, p_discount_amt, p_total, p_payment_method, p_customer_name, p_sold_by, p_sold_by_email);
END;
$$;

-- Public record_sale: text (client sends JSON string; ensures deduction works when serialization differs).
CREATE OR REPLACE FUNCTION record_sale(
  p_warehouse_id    uuid,
  p_lines           text,
  p_subtotal        numeric,
...
BEGIN
  ...
  RETURN record_sale_impl(p_warehouse_id, v_lines_jsonb, p_subtotal, p_discount_pct, p_discount_amt, p_total, p_payment_method, p_customer_name, p_sold_by, p_sold_by_email);
END;
$$;

COMMENT ON FUNCTION record_sale_impl(uuid, jsonb, numeric, numeric, numeric, numeric, text, text, uuid, text) IS
  'Internal: atomic sale with stock deduction. Guard empty lines; sized products require size_code.';
...
GRANT EXECUTE ON FUNCTION record_sale(uuid, text, numeric, numeric, numeric, numeric, text, text, uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION record_sale(uuid, text, numeric, numeric, numeric, numeric, text, text, uuid, text) TO service_role;
