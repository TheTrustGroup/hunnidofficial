    ...
    >
      {tabs.map((item) => (
        <NavLink
          ...
          to={item.to}
          className={({ isActive }) =>
            `flex min-h-[44px] min-w-[44px] flex-col items-center justify-center gap-0.5 px-2 py-1.5 rounded-xl transition-colors touch-manipulation ${
              isActive
      ...
      {showMore && (
        <button
          type="button"
          onClick={onMoreClick}
          className="flex min-h-[44px] min-w-[44px] flex-col items-center justify-center gap-0.5 px-2 py-1.5 rounded-xl text-[var(--edk-ink-2)] hover:bg-[var(--edk-bg)] transition-colors touch-manipulation"
          role="tab"
