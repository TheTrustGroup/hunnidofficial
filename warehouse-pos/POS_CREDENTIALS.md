...
|--------|--------|----------|
| Admin / full access | `admin@hunnidofficial.com` | *(your existing admin password)* |

The server treats `admin@hunnidofficial.com` as admin (see `ALLOWED_ADMIN_EMAILS` or default in `inventory-server/lib/auth/roles.ts`). Admin password is not checked by the warehouse app (set in your main admin if needed).
...
|----------|--------|----------|
| **Main Store / DC** | `jcashier@hunnidofficial.com` | `MEDk-1!@#` |
| **Main Town** | `maintown_jcashier@hunnidofficial.com` | `TEDk-2!@#` |
