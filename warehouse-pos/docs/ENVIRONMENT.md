...
| `ALLOWED_ORIGIN_SUFFIXES` | No | Comma-separated hostname suffixes (e.g. `.vercel.app`). Request origin is allowed if its hostname ends with one. Defaults include `.vercel.app`, `.hunnidofficial.com` so health and API work from Vercel frontends. |
