...
| `POS_PASSWORD_CASHIER_MAIN_STORE` | For POS | Password for `jcashier@hunnidofficial.com`. |
| `POS_PASSWORD_MAIN_TOWN` | For POS | Password for `maintown_jcashier@hunnidofficial.com`. |
...
3. **Custom domain**  
   - In Vercel (or your host), set the frontend domain to `warehouse.hunnidofficial.com` (or your chosen domain).  
   - Ensure API CORS allows that origin (`lib/cors.ts` already allows `https://warehouse.hunnidofficial.com` by default).
...
2. **Login**  
   Open frontend → Log in with admin (e.g. admin@hunnidofficial.com) or POS (cashier@… / maintown_cashier@… with env-set passwords). No 401/503.
