# How APIs Should Be Set: Warehouse POS, Inventory Server, hunnidofficial.com

This doc explains how the **warehouse-pos** frontend, the **inventory-server** API, and the **hunnidofficial.com** domain should be wired so login, products, warehouses, and orders work.
...
|-------|------------|----------------|
| **Warehouse POS** | Frontend app (React). Users open it in the browser. | Typically **https://warehouse.hunnidofficial.com** |
| **Inventory server** | Backend API (Next.js in this repo). Serves `/api/*` and `/admin/api/*`. | Must be reachable at one base URL (see below). |
| **hunnidofficial.com** | Your main domain. Can be the same as the API or a different app. | Depends on your deployment. |
...
- If the inventory-server is at **https://hunnidofficial.com** → set `VITE_API_BASE_URL=https://hunnidofficial.com`.
- If the inventory-server is at **https://api.hunnidofficial.com** → set `VITE_API_BASE_URL=https://api.hunnidofficial.com`.
...
### Setup A: API on the main domain (hunnidofficial.com = inventory-server)

- **hunnidofficial.com** is the **inventory-server** Vercel project (or that project is assigned to this domain).
- The warehouse app calls **https://hunnidofficial.com/api/...** and **https://hunnidofficial.com/admin/api/...**.

**Env:**

| Project | Variable | Value |
|---------|----------|--------|
| **Warehouse POS** (warehouse.hunnidofficial.com) | `VITE_API_BASE_URL` | `https://hunnidofficial.com` |
| **Warehouse POS** | `VITE_SUPER_ADMIN_EMAILS` | Your admin email(s), comma-separated (optional but recommended) |
| **Inventory server** (hunnidofficial.com) | `CORS_ORIGINS` | `https://warehouse.hunnidofficial.com,https://hunnidofficial.com` |
...
### Setup B: API on a subdomain (api.hunnidofficial.com = inventory-server)

- **hunnidofficial.com** is a different app (e.g. main site).
- **api.hunnidofficial.com** (or another URL) is the **inventory-server** project.

**Env:**

| Project | Variable | Value |
|---------|----------|--------|
| **Warehouse POS** (warehouse.hunnidofficial.com) | `VITE_API_BASE_URL` | `https://api.hunnidofficial.com` (or the real inventory-server URL) |
| **Warehouse POS** | `VITE_SUPER_ADMIN_EMAILS` | Your admin email(s) (optional but recommended) |
| **Inventory server** (api.hunnidofficial.com) | `CORS_ORIGINS` | `https://warehouse.hunnidofficial.com,https://hunnidofficial.com` (if main site also calls API) |
| **Inventory server** | (others) | Same as above. |

Result: warehouse app calls the API subdomain; CORS must allow `https://warehouse.hunnidofficial.com`.
...
- [ ] **`VITE_API_BASE_URL`** = full URL of the **inventory-server** including **`https://`** (e.g. `https://warehouse-pos-api-v2-xxxx.vercel.app`). No trailing slash. Without the protocol, login requests go to the wrong host (405 / invalid credentials).  
  Examples: `https://hunnidofficial.com` or `https://api.hunnidofficial.com`.
...
- [ ] **`CORS_ORIGINS`** (or **`FRONTEND_ORIGIN`**) includes `https://warehouse.hunnidofficial.com` so the browser allows requests from the warehouse app.
...
- **hunnidofficial.com** can be that app (Setup A) or a different app; if different, the API must live at another URL and the warehouse must point there.
